...
### Get "NOT_AUTHORIZED" from Polygon API if running `trader` when it's one day ahead in UTC time than New York time:
...
The cutoff of \`get\_previous\_close\_agg\`  is UTC time.  \`get\_grouped\_daily\_aggs\` is checking using trading date which is NYC time. There is the 4 or 5 hours lag depending upon if daylight saving is involved (Currently it's 5 hours lag/ gap).
...
`with sqlite3.connect(DB) as conn` in the front of database.py is supposed to execute before I call any function/ def.  It did not. I kept getting that error. The funny thing is that account.db was created after work but just not before `get_share_price` call.  I added `class` and `__init__` to ensure tables were created before any functions.  Yes, market.py, accounts.py and tracers.py all initialize Database first.
...
```### I kept getting Error: Max turns (10) exceeded on `Researcher` even thought MAX_TURNS = 30 is configured
...
```
**Notice that the above only regulates 'trader' agent but not 'researcher` agent**  Runner is not running 'researcher' agent directly and 'researcher' agent run indirectly as a tool for 'trader' agent.  We can regulate the max_turn on the tool.
